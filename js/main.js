...
$(document).ready(function() {
	...
	$(".js_r_preview").click(function(){ 
		loadPopup('.rout_user_pupup');
	 	...
	 	var langeBookmark = $('.js_share_map_list:visible').find('.js_share_map_but').length;
	 	var WidthBookmark;
	 	if (langeBookmark > 3) {
   ...
   $('.route_day_content .route_day:nth-child(3n)').css({marginRight : 0});
	...
	$('.rout_user_box_text').click(function() {
		var ClickBoxNamber = $(this).parents('.trails-tab-content').find('.rout_user_box').length;
		var boxHeight = $(this).parents('.trails-tab-content').find('.rout_user_box_cont');
		...
		if (ClickBoxNamber > 3) {boxHeight.css({'height' : 397 + 'px'});};
